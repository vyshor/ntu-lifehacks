<template>
    <v-container grid-list-md text-xs-center>
        <v-form id="bookingForm">
            <v-layout row wrap>
                <v-flex xs6>
                    <v-select :items="locations" label="Location" v-model="locationIn" v-on:change="getFacilities"></v-select>
                </v-flex>
                <v-flex xs6>
            ...
            <v-layout row wrap>
                ...
                <v-flex xs4>
                    <v-select :items="timeslots" label="Start" item-text="end" item-value="id"></v-select>
                </v-flex>
            </v-layout>
            <v-layout row wrap>
                <v-spacer></v-spacer>
...
<script>
    ...
    export default {
        ...
        mounted () {
            ...
            //facilities data
            axios.get("http://192.168.31.167/booking/library/facilities")
                .then(response => {
        ...
        methods: {
            getBookings (facId) {
                ...
                axios.get("http://192.168.31.167/booking/library/" + this.facilitydata[facId])
                    .then(response => {
